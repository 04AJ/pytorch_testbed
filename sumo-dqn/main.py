...
#     else:
#         return torch.tensor([[env.choice_list.sample()]], device=device, dtype=torch.long)
...
for i_episode in range(num_episodes):
    ...
    total_reward = 0
    for t in count():
        taction, action = select_action(state)
        # env in stepped
        try:
            observation, reward, terminated, info = env.step(action)
        except traci.exceptions.TraCIException:
                print("Got some exception here!")
                break
        total_reward += reward
        ...
        if done:
            ...
            log_string = "episode: {}/{}, score: {}, last edge: {}\n".format(
                    i_episode, num_episodes, total_reward, info)
            print(log_string)
